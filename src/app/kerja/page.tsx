  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-16">
      <section className="mb-10 max-w-3xl space-y-4">
        <p className="text-sm font-medium text-primary">{t("workBadge")}</p>
        ...
        <p className="text-sm font-medium">
          {t("focusLabel")}: {t("workFocus")}
        </p>
      </section>

      <JourneyFlowNav audience="work" currentStep="backstory" />

      <Card className="border-primary/20 bg-primary/5">
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <CardTitle>{tJourney("hubStartTitle")}</CardTitle>
            <CardDescription>{tJourney("workHubStartDesc")}</CardDescription>
          </div>
          <Button render={<Link href={getStepPath("work", "backstory")} />}>
